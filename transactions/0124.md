...
| 40     | 8    | 8-byte    | PrevSeq               | XXH64 of previous chain state; `0` = unset (proxy-stamped) |
| 48     | 8    | 8-byte    | CurSeq                | XXH64 of current chain state; `0` = unset (proxy-stamped)  |
...
An 8-byte XXH64 hash representing the previous frame's chain state for this sender+group flow, stamped in-place by the proxy (`bitcoin-shard-proxy`) before multicast forwarding. Senders set this field to `0`. A value of `0` means the proxy has not stamped the frame.

`PrevSeq` of frame *N* equals `CurSeq` of frame *N−1*. A mismatch between the incoming `PrevSeq` and the receiver's `lastCurSeq` indicates one or more missing frames, triggering NACK-based retransmission.

#### CurSeq (bytes 48–55)

An 8-byte XXH64 hash representing the current frame's chain state, stamped in-place by the proxy. Computed as:

```
CurSeq = XXH64(senderIPv6[16] ∥ groupIdx[4] ∥ counter[8])
```

where `counter` starts at 1 and increments monotonically per `(senderIPv6, groupIdx)` pair. A value of `0` means the frame has not been stamped and must be discarded by the retry endpoint ingress. Receivers use `CurSeq` as the primary cache key for NACK lookup; `PrevSeq` provides a secondary forward-lookup index.
...
0000000000000000                                                  // PrevSeq (0 = first frame in chain)
A3C5D2E1B4F6C789                                                  // CurSeq (proxy-stamped XXH64)
