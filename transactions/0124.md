...
4. **Hash-chain sequencing** — stamping `PrevSeq`/`CurSeq` (XXH64) for gap detection and NACK-based retransmission
...
| 40     | 8    | 8-byte    | PrevSeq          | XXH64 of previous chain state; `0` = first frame or unstamped |
| 48     | 8    | 8-byte    | CurSeq           | XXH64 of current chain state; `0` = unstamped                 |
...
A 64-bit unsigned integer (big-endian) containing an XXH64 hash of the previous frame's chain state for this `(senderIPv6, groupIdx)` pair. Equals the `CurSeq` of the immediately preceding frame in the chain. Set either by the sender or stamped in-place by the ingress proxy (see §Proxy Forward Rules). A value of `0` indicates the first frame in a sequence or that the field is unstamped.

Receivers use this field to detect gaps: if the incoming `PrevSeq` does not match the locally stored `lastCurSeq` for that group, a frame was lost and NACK-based recovery is triggered.

#### CurSeq (bytes 48–55)

A 64-bit unsigned integer (big-endian) containing an XXH64 hash of the current frame's chain state. Set either by the sender or stamped in-place by the ingress proxy. A value of `0` indicates the field is unstamped.

The hash input is `senderIPv6 (16 B) ‖ groupIdx (4 B BE) ‖ monotonicCounter (8 B BE)`, hashed with XXH64 (`github.com/cespare/xxhash/v2`). Receivers use `CurSeq` as the primary cache key for NACK-based retransmission requests.
...
Legacy BRC-12 frames (Frame Version `0x01`) use a 44-byte header with no `PrevSeq`, `CurSeq`, or `SubtreeID` fields:
...
0000000000000000                                                  // PrevSeq (first frame; unset)
1A2B3C4D5E6F7A8C                                                  // CurSeq (XXH64 chain value)
